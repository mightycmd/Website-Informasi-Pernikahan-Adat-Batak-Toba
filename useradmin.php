...
if (isset($_POST['tambahuser'])) {
	...
	$query = "SELECT * FROM user_admin WHERE username = '$username' OR email = '$email'";
	...
	if ($row > 0) {
		...
		$sql = mysqli_query($connect, "INSERT INTO user_admin VALUES ('', '$email', '$nama', '$username', '$password')");
...
if (isset($_POST['edituser'])) {
	...
	$query = "UPDATE user_admin SET email= '$email', Nama= '$nama', username= '$username' WHERE id = '$id'";
...
<form class="text-left" action="" method="POST">
	...
	<fieldset class="border p-2">
		<legend  class="w-auto">Tambah User</legend>
...
<fieldset class="border p-2 mt-4">
		...
		<table class="table">
		  	...
		  	<?php 
		  		$i = 1;
		  		$sqlAdmin = "SELECT * FROM user_admin ORDER BY id ASC"; 
		    ...
		    <tr>
		      ...
		      <td> <a href="?mod=useradmin&act=edit&id=<?php echo $row['id']; ?>">Edit</a> / <a href="?mod=useradmin&act=hapus&id=<?php echo $row['id']; ?>">Hapus</a></td>
		    </tr>
		    <?php $i++; ?>
			<?php endwhile; ?>
		  	</tbody>
